...
const loadPortfolios = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/portfolio/portfolios?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    error.value = err.message || 'Failed to load portfolios';
  }
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadPortfolios(currentPage.value + 1);
  }
};
...
const getPortfolioIcon = (portfolio: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[portfolio.portfolioId % icons.length];
};

const viewPortfolioDetails = (portfolio: any) => {
...
<template>
  <div class="portfolios-page">
    ...
    <header class="portfolios-header">
      <div class="header-row">
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              <h1 class="page-title">Portfolios</h1>
              <div class="portfolios-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="portfolios-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Portfolios</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadPortfolios()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredPortfolios.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Portfolios Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No portfolios match your search criteria.' : 'No portfolios found in the system.' }}
        </p>
      </div>

      <!-- Portfolios Grid -->
      <div v-else-if="viewMode === 'cards'" class="portfolios-grid">
        <div
        ...
        >
          <div class="portfolio-header">
            <div class="portfolio-icon">
              {{ getPortfolioIcon(portfolio) }}
            </div>
          </div>

          <div class="portfolio-info">
            <h3 class="portfolio-name">{{ portfolio.name || 'Unknown Portfolio' }}</h3>
            ...
            <div class="portfolio-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">PortfolioId:</span>
                <span class="metadata-value">{{ portfolio.portfolioId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">PortfolioName:</span>
                <span class="metadata-value">{{ portfolio.portfolioName || 'N/A' }}</span>
              </div>
      ...
      <!-- Portfolios Table -->
      <div v-else-if="viewMode === 'table'" class="portfolios-table-container">
        <table class="portfolios-table">
          <thead>
            <tr>

              <th @click="handleSort('portfolioId')" class="sortable-header">
                PortfolioId {{ getSortIcon('portfolioId') }}
              </th>
              <th @click="handleSort('portfolioName')" class="sortable-header">
                PortfolioName {{ getSortIcon('portfolioName') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="portfolio in filteredPortfolios" :key="portfolio.portfolioId" class="portfolio-row">
              
              
                      <td class="portfolio-id-cell" @click="viewPortfolioDetails(portfolio)">{{ portfolio.portfolioId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ portfolio.portfolioName }}</span>
                </div>
              </td>
